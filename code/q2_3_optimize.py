...
def saved_solution(ctx: Context, output_root: Path,
                   methods: tuple[str, ...] = ("opt1", "opt2")) -> tuple[dict[str, Any] | None, str | None]:
    """仅使用同一份已验收输入且通过全部硬约束的本地历史方案。"""
    source_hash = sha256(ctx.data_run / "meta" / "validated_artifacts.json")
    locations = {
        "opt1": ("1_optimize", "q2_opt_*_table"),
        "opt2": ("2_optimize", "q2_opt2_*_table"),
        "opt3": ("3_optimize", "q2_opt3_*_table"),
    }
    directories = [directory for method in methods
                   for root in (output_root / locations[method][0], output_root)
                   for directory in root.glob(locations[method][1])]
    valid = []
    for directory in sorted(directories, key=lambda p: p.stat().st_mtime, reverse=True):
        ...
        if (is_admissible(plan, 0.0) and
            ...
            valid.append((plan, str(directory)))
    return (min(valid, key=lambda item: (item[0]["score"][4],
                                          item[0]["score"][3]))
            if valid else (None, None))


def initial_solution(ctx: Context, output_root: Path,
                     methods: tuple[str, ...] = ("opt1", "opt2")) -> tuple[dict[str, Any], str]:
    warm, origin = saved_solution(ctx, output_root, methods)
...
def save_tradeoff_figure(figure_dir: Path, solutions: dict[str, dict[str, Any]]) -> None:
    metrics = {(round(plan["score"][3], 6), round(plan["score"][4], 6),
                plan["score"][5], round(plan["score"][2], 6))
               for plan in solutions.values()}
    if len(metrics) == 1:
        with (figure_dir / "图表说明.txt").open("a", encoding="utf-8") as stream:
            stream.write("三个搜索档位得到同一方案，因此不生成重合且无信息量的权衡图。\n")
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="问题二优化方案3：大邻域搜索与三种权衡")
    ...
    method_dir = output_root / "3_optimize"
    table_dir = method_dir / f"q2_opt3_{stamp}_table"
    figure_dir = method_dir / f"q2_opt3_{stamp}_figure"
