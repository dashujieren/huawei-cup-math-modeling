...
from q1_2_plot_results import CODE_DIR, FIGURES, configure_plot_style
...
def latest_optimized() -> Path:
    for directory in sorted((CODE_DIR / "1_outputs").glob("q1_opt_*_table"), reverse=True):
...
def main() -> None:
    ...
    output_dir = args.output_dir.resolve() if args.output_dir else (
        FIGURES / datetime.now().strftime("q1_opt_%y%m%d_%H%M%S_figure"))
