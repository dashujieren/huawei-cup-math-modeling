...
from pipeline import sha256, verify_ready
...
def save_results(result: dict[str, Any], output_dir: Path) -> None:
    ...
    template = load_workbook(PROJECT / "结果提交模板.xlsx", read_only=True, data_only=True)
...
def find_latest_validated_run() -> Path:
    for path in sorted((CODE_DIR / "outputs").glob("run_*"), reverse=True):
        if path.is_dir() and verify_ready(path):
            return path
    raise FileNotFoundError("找不到通过验收的数据目录，请先运行 run_all.py")
...
def main() -> None:
    ...
    output_dir = args.output_dir.resolve() if args.output_dir else (
        CODE_DIR / "outputs" / datetime.now().strftime("q1_baseline_%Y%m%d_%H%M%S_%f"))
