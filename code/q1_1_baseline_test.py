...
def test_one_type_unreachable_does_not_invalidate_feasible_problem(monkeypatch):
    ...
    monkeypatch.setattr(q1_0_baseline, "_read_inputs", lambda _: (boxes, {"A": a, "B": b, "C": c}, arcs))
...
def test_official_data_baseline_replays_all_boxes():
    runs = sorted((CODE_DIR / "0_outputs").glob("run_*"), reverse=True)
    if not runs:
...
def test_export_reopens_csv_before_ready(tmp_path):
    ...
    assert list(pd.read_csv(destination / "Q1_单点组批.csv").columns) == q1_0_baseline.TEMPLATE_COLUMNS
