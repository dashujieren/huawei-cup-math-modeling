...
from pipeline import extract, geo, normalize, record_failure, validate, verify_ready


def main() -> None:
    project = Path(__file__).resolve().parent.parent
    run_dir = Path(__file__).resolve().parent / "outputs" / datetime.now().strftime("run_%Y%m%d_%H%M%S")
    if run_dir.exists():
