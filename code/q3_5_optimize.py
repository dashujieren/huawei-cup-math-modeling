"""问题三最终时间优化方案：从方案三少架次 PASS 解缩短最晚返航时间。

在 PyCharm 中直接运行本文件；工作目录设为 code，脚本形参可留空。
同目录的 q3_1_optimize、q3_2_optimize、q3_3_optimize 和
q2_1_optimize 等文件仍为模型依赖，不需要单独启动。
...
def _save_result(args: argparse.Namespace, origin: Path,
    ...
    result = {
        ...
        "code_sha256": {name: base.sha256(CODE_DIR / name) for name in
                        ("q3_5_optimize.py", "q3_3_optimize.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-root", type=Path,
                        default=CODE_DIR / "3_outputs" / "5_optimize")
